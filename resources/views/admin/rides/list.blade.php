    ...
    <script>
        $(document).ready(function() {
            var table = new Tabulator("#example-table", {
                ajaxURL: "{{ route('admin.rides.index') }}",
                ajaxConfig: "GET",
                pagination: "remote",
                paginationSize: 10,
            ...
            // Delegated event because buttons are dynamically generated
            $(document).on('click', '.view-car', function() {
                var carData = $(this).data('car');

                // Set data inside modal
                $('#car-brand').text(carData.brand || 'N/A');
                $('#car-model').text(carData.model || 'N/A');
                $('#car-interior').text(carData.interior || 'N/A');
                $('#car-seats').text(carData.seats || 'N/A');
                $('#car-register-number').text(carData.register_number || 'N/A');
